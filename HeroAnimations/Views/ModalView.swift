...
struct ModalMod: AnimatableModifier {
    ...
    @Environment(\.favRadiusPct) var favRadiucPct: CGFloat
    ...
    func body(content: Content) -> some View {
        ...
        let refRadiusPct = flyingFromGrid ? gridRadiusPct : favRadiucPct
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(alignment: .top, spacing: 0) {
                    Image(item.image)
                        ...
                        .padding(4 * pct)
                        .frame(width: (pictureSize.width - proxy.size.width) * pct + proxy.size.width, height: (pictureSize.height - proxy.size.height) * pct + proxy.size.height)
                    
                    VStack(alignment: .leading) {
                        Text(item.name).font(.largeTitle)
                            .fontWeight(.heavy).foregroundColor(Color(UIColor.label))
                        Text(item.class).font(.none).fontWeight(.bold)
